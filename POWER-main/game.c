...
bool verifier_case_juste(int x, int y, S_plateau plateau, S_pions pion, int ancienne_couleur, int * nb_chgmt_couleur)
{
    ...
    if(nouvelle_couleur!=ancienne_couleur) //s'il y a eu changement de couleur du pion
    {
        *nb_chgmt_couleur ++;
        if(*nb_chgmt_couleur>1) //s'il y a eu plusieurs changements de couleurs du pion
        {
            return false;
        }
    }
...
bool verifier_chemin(S_plateau plateau, S_pions pion, int x_destination, int y_destination)
{
    ...
    float add_x = dx/nombre_de_tour; //déplacement à chaque tour sur l'axe X (peut être plus petit que 1)
    float add_y = dy/nombre_de_tour; //déplacement à chaque tour sur l'axe Y (peut être plus petit que 1)
